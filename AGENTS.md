...
- **Refonte Communauté « la saison »** : EN COURS (décidée avec Louis le 2026-07-18, maquette v4 validée en discussion — dernier chantier avant la refonte de l'accueil). Concept : cercle privé + **boucle de saison mensuelle** façon jeu vidéo. Deux moteurs : (1) **le Duel** — deux camps renouvelés chaque mois (ex. Solaire ☀ / Lunaire ☾), chaque séance terminée = 1 point pour ton camp, cérémonie de choix plein écran le 1ᵉʳ du mois (réf. Fortnite Ghost/Shadow : deux mondes, maintenir 2 s pour sceller, AUCUNE stat avant le choix) ; (2) **l'Exploit hebdo** — défi perso sur l'honneur (badge nommé à vie + halo doré 24 h). S'y ajoutent : **rangs** Bronze→Argent→Or→Platine→Diamant→✦ Étincelle nourris par « l'éclat » (constance UNIQUEMENT — jamais le corps ni la perf, pilier zéro culpabilisation), reset doux par saison (tous à Argent), classements **entre amis seulement** (jamais globaux), **duels de cercle** (famille/amis/travail — une séance marque dans TOUTES les arènes du joueur), badge fondateur par vagues (`profiles.member_number` : 100/500/2000), finale mini-Wrapped partageable.
  ...
  **Déjà codé sur `dev`** (`seasonApi.ts`/`useSeason.ts` = accès données, `SeasonCeremony.tsx` monté dans layout, `season/SeasonHQ.tsx` = écran d'arrivée de l'onglet, `season/SeasonProgression.tsx` = overlay rang/classement/badges, crédit d'éclats dans `WorkoutGuideModal`, événements `rank_up`/`exploit_done`) + **grand ménage fait** (c30f982) : le feed TikTok immersif et la recherche de contenu ont été SUPPRIMÉS — la recherche = « Trouver des gens » (personnes uniquement), les vidéos sont des posts normaux du fil. **Reste** : finale de saison (mini-Wrapped + clôture/vainqueur + reset doux), événement `duel_lead`, rangée de présence (avatars du cercle), duels de cercle (UI), admin de création saisons/exploits. ⚠️ `communaute/page.tsx` = fichier monolithe : un seul agent à la fois.
