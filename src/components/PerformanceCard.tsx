...
export type FontStyle = "light" | "regular" | "bold";

const fontWeights: Record<FontStyle, { hero: number; label: number; sub: number }> = {
  light:   { hero: 300, label: 300, sub: 400 },
  regular: { hero: 700, label: 600, sub: 700 },
  bold:    { hero: 900, label: 700, sub: 800 },
};
...
export default function PerformanceCard({
  ...
  const fw = fontWeights[fontStyle];
  ...
  return (
    <motion.div
      ...
      className={`relative overflow-hidden flex flex-col ${px} ${py}`}
      style={{
        borderRadius: isSmall ? Math.min(cardRadius, 20) : cardRadius,
      ...
      {hero && (
        ...
        >
          ...
          <div style={{ display: "flex", alignItems: "flex-end", gap: "0.5rem", marginBottom: "0.5rem", justifyContent: centered ? "center" : "flex-start" }}>
            <span style={{ fontWeight: fw.hero, lineHeight: 1, color: "#fff", fontSize: heroSz, letterSpacing: "-0.02em", textShadow: ts }}>
              {hero.value}
            </span>
            {hero.unit && (
