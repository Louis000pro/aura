  ...
  return (
    <div className="mb-5">
      <div className="flex items-center gap-1.5 mb-2">
        <BookOpen size={11} strokeWidth={2} style={{ color: "#A0AEC0" }} />
      ...
      <div className="grid grid-cols-2 gap-2">
        {THEMES.map((t) => (
          <motion.button key={t.label} whileTap={{ scale: 0.96 }} whileHover={{ scale: 1.02 }}
            ...
            <span className="text-xs font-medium leading-tight" style={{ color: "#2D3748" }}>{t.label}</span>
          </motion.button>
        ))}
      </div>
        ...
        {open && (
          ...
          >
            <motion.div
              ...
              style={{ maxHeight: "88vh", boxShadow: "0 -8px 40px rgba(167,139,250,0.18)" }}
            >
              <div className="flex items-center justify-between px-5 pt-5 pb-3" style={{ borderBottom: "1px solid rgba(212,192,255,0.4)" }}>
                <div>
                  <p className="text-[10px] font-semibold tracking-widest uppercase" style={{ color: "#A0AEC0" }}>{activeTheme}</p>
                  <h2 className="text-base font-semibold mt-0.5" style={{ color: "#2D3748" }}>
                ...
                <button onClick={() => setOpen(false)} aria-label="Fermer"
                  ...
                  <X size={15} strokeWidth={2} style={{ color: "#718096" }} />
                </button>
              </div>
              ...
              <div className="flex-1 min-h-0 overflow-y-auto px-5 py-4" style={{ scrollbarWidth: "none" }}>
                {loading && (
                  <div className="flex flex-col items-center justify-center gap-3 py-16">
                    ...
                    <p className="text-xs" style={{ color: "#A0AEC0" }}>Ingrédients, étapes et macros…</p>
                  </div>
                )}
                ...
                {error && !loading && (
                  <div className="flex flex-col items-center justify-center gap-3 py-14">
                    ...
                    <p className="text-sm text-center" style={{ color: "#718096" }}>L&apos;IA a calé cette fois.</p>
                    <button onClick={() => activeTheme && generate(activeTheme)}
                      className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold"
                      style={{ background: "rgba(240,235,255,0.8)", color: "#7C5CFA" }}>
                ...
                {recipe && !loading && (
                  <div className="flex flex-col gap-4">
                    <div className="flex flex-wrap gap-1.5">
                      <span className="text-[11px] px-2.5 py-1 rounded-full font-medium" style={{ background: "rgba(240,235,255,0.8)", color: "#7C5CFA" }}>{recipe.portions} portion{recipe.portions > 1 ? "s" : ""}</span>
                      <span className="text-[11px] px-2.5 py-1 rounded-full font-medium" style={{ background: "rgba(240,235,255,0.8)", color: "#7C5CFA" }}>prépa {recipe.prepMin} min</span>
                      {recipe.cookMin > 0 && <span className="text-[11px] px-2.5 py-1 rounded-full font-medium" style={{ background: "rgba(240,235,255,0.8)", color: "#7C5CFA" }}>cuisson {recipe.cookMin} min</span>}
                      <span className="text-[11px] px-2.5 py-1 rounded-full font-medium" style={{ background: "rgba(240,235,255,0.8)", color: "#7C5CFA" }}>{recipe.difficulty}</span>
                      {recipe.diet.map((d) => (
                    ...
                    <div className="flex items-center justify-around py-3 rounded-2xl" style={{ background: "rgba(240,235,255,0.5)" }}>
                      {[
                        ...
                        <div key={m.l} className="flex flex-col items-center">
                          <span className="text-sm font-bold" style={{ color: "#2D3748" }}>{m.v}</span>
                          <span className="text-[9px]" style={{ color: "#A0AEC0" }}>{m.l}</span>
                        </div>
                      ))}
                    </div>
                    ...
                    <div>
                      <p className="text-[10px] font-semibold tracking-widest uppercase mb-2" style={{ color: "#A0AEC0" }}>Ingrédients</p>
                      <div className="flex flex-col gap-1.5">
                        {recipe.ingredients.map((it, i) => (
                          <div key={i} className="flex items-center justify-between text-sm">
                            <span style={{ color: "#2D3748" }}>{it.nom}</span>
                            <span className="font-medium" style={{ color: "#A78BFA" }}>{it.quantite}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    <div>
                      <p className="text-[10px] font-semibold tracking-widest uppercase mb-2" style={{ color: "#A0AEC0" }}>Préparation</p>
                      <div className="flex flex-col gap-2.5">
                        {recipe.steps.map((s, i) => (
                          <div key={i} className="flex gap-2.5">
                            <span className="flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center text-[11px] font-bold" style={{ background: "rgba(167,139,250,0.15)", color: "#7C5CFA" }}>{i + 1}</span>
                            <p className="text-sm leading-snug" style={{ color: "#4A5568" }}>{s}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    {recipe.safetyNote && (
                      <p className="text-[11px] italic" style={{ color: "#A0AEC0" }}>{recipe.safetyNote}</p>
                    )}
                  </div>
                )}
              </div>
              ...
              {recipe && !loading && (
                <div className="px-5 pt-3" style={{ borderTop: "1px solid rgba(212,192,255,0.4)", paddingBottom: "calc(1rem + env(safe-area-inset-bottom))" }}>
