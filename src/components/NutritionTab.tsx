  ...
  return (
    <div className="relative flex-shrink-0" style={{ width: 216, height: 216 }}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center gap-1">
        <p className="vy-label" style={{ color: "var(--text-3)" }}>Consommé</p>
        <motion.p
          initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}
          transition={{ delay: 0.4 }}
          /* ⚠️ PAS `.vy-nombre` ICI, ET C'EST UNE MESURE, PAS UN GOUT.
             `toLocaleString("fr-FR")` separe les milliers par une espace
             insecable ; dans une chasse fixe cette espace occupe une CASE
             ENTIERE, donc « 1 480 » se lit « 1 » puis « 480 », c'est-a-dire
             deux valeurs au lieu d'une. Verifie a l'ecran a 40 px. La famille
             du chiffre sert les valeurs qu'on regarde, mais pas celles qui
             portent un separateur de milliers francais. */
    ...
    <div>
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px]" style={{ color: "var(--text-3)" }}>{pct}%</span>
        </div>
        <span className="text-[16px] font-semibold" style={{ color: "var(--text-1)" }}>
          {consumed}g <span className="font-normal text-[11px]" style={{ color: "var(--text-3)" }}>/ {goal}g</span>
        </span>
      </div>
      {hint && (
          ...
          <div className="flex items-center gap-2.5 min-w-0">
            {onBack && (
              <motion.button whileTap={{ scale: 0.9 }} onClick={onBack}
                ...
                <ChevronLeft size={16} strokeWidth={2.2} style={{ color: "var(--exp-encre)" }} />
              </motion.button>
            )}
            <div className="min-w-0">
              <p className="vy-label" style={{ color: "var(--text-3)" }}>{onBack ? "Retour à la carte" : "Nutrition"}</p>
              <h2 className="text-[20px] font-semibold truncate" style={{ color: "var(--text-1)" }}>
          ...
          <AnimatePresence mode="wait">
            ...
            {phase === "select" && (
              <motion.div key="select"
                ...
                {error && (
                  <div className="mb-3 px-3 py-2.5 rounded-2xl text-[13px] font-medium"
                  ...
                  {camReady ? (
                    ...
                    <button type="button" onClick={() => camRef.current?.click()}
                      ...
                      <div className="text-center px-6">
                        <p className="text-[16px] font-semibold" style={{ color: "#fff" }}>Prendre une photo</p>
                ...
                {camReady ? (
                  ...
                  <div className="flex items-center justify-between mt-4 px-6">
                    <motion.button whileTap={{ scale: 0.9 }} onClick={() => fileRef.current?.click()}
                      ...
                      <ImageIcon size={19} strokeWidth={1.8} style={{ color: "var(--exp-encre)" }} />
                    </motion.button>
                    <motion.button whileTap={{ scale: 0.92 }} onClick={capturePhoto}
                      ...
                      <span className="rounded-full" style={{ width: 52, height: 52, background: "linear-gradient(135deg,#8B5CF6,#C13BC1)", boxShadow: "var(--ombre-action)" }} />
                    </motion.button>
                    <motion.button whileTap={{ scale: 0.9 }} onClick={flipCam}
                      className="w-11 h-11 rounded-2xl flex items-center justify-center cursor-pointer"
                      style={{ background: "rgba(var(--tint-violet-rgb),0.7)", border: "1px solid rgba(var(--violet-mid-rgb),0.5)" }}>
                      <SwitchCamera size={19} strokeWidth={1.8} style={{ color: "var(--exp-encre)" }} />
                    </motion.button>
                  </div>
                ) : (
                  /* Fallback : galerie */
                  <motion.button whileTap={{ scale: 0.97 }} onClick={() => fileRef.current?.click()}
                    ...
                    <Upload size={15} strokeWidth={1.8} style={{ color: "var(--exp-encre)" }} />
                    <span className="font-medium text-[16px]" style={{ color: "var(--text-2)" }}>Choisir dans la galerie</span>
                  </motion.button>
                )}
                  ...
                  <div className="w-full rounded-2xl overflow-hidden relative" style={{ height: 300 }}>
                    ...
                    <div className="absolute left-0 right-0 flex items-center justify-center gap-2" style={{ bottom: 16 }}>
                      ...
                      <motion.span animate={{ opacity: [0.6, 1, 0.6] }} transition={{ duration: 1.6, repeat: Infinity }}
                        className="text-[11px] font-semibold" style={{ color: "#fff" }}>
              ...
              <motion.div key="result"
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <p className="vy-label flex items-center gap-1" style={{ color: "var(--text-3)" }}>
                      <Check size={11} strokeWidth={3} style={{ color: "var(--teal-encre)" }} /> Repas identifié
                    </p>
                    <p className="font-semibold text-[16px] leading-tight mt-1" style={{ color: "var(--text-1)" }}>
                      {editData.foodName}
                    </p>
                    {editData.description && (
                      <p className="text-[13px] mt-0.5 font-light" style={{ color: "var(--text-2)" }}>
                        {editData.description}
                      </p>
                    )}
                  </div>
                  <div className="text-right flex-shrink-0">
                    <p className="vy-nombre text-[26px] leading-none" style={{ fontWeight: 500, color: "var(--exp-encre)" }}>{editData.calories}</p>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <motion.button whileTap={{ scale: 0.9 }} onClick={() => setPhase("edit")}
                    className="flex items-center gap-1 text-[13px] font-medium cursor-pointer" style={{ color: "var(--exp-encre)" }}>
                ...
                <div className="flex gap-2 mt-0.5">
                  <motion.button whileTap={{ scale: 0.95 }} onClick={reset}
                    className="flex-1 py-3 rounded-2xl text-[13px] font-medium cursor-pointer"
                  ...
                  <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}
                    ...
                    className="flex-[2] py-3 rounded-2xl text-[16px] font-bold cursor-pointer flex items-center justify-center gap-2"
            ...
            {phase === "edit" && editData && (
              <motion.div key="edit"
                ...
                {[
                  ...
                  <div key={key}>
                    <label className="vy-label mb-1 block"
                    ...
                    <input
                      ...
                      className="w-full px-4 py-2.5 rounded-xl text-[16px] outline-none"
                ...
                <div>
                  <label className="vy-label mb-1.5 block"
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {(Object.keys(MEAL_META) as MealType[]).map(mt => (
                      <motion.button key={mt} whileTap={{ scale: 0.95 }}
                        ...
                        className="py-2 rounded-xl text-[11px] font-medium cursor-pointer flex items-center justify-center gap-1.5"
                        style={{
                ...
                <motion.button whileTap={{ scale: 0.97 }}
                  ...
                  className="w-full py-3 rounded-2xl text-[16px] font-bold cursor-pointer mt-1"
                  style={{
          ...
          <AnimatePresence mode="wait">
            ...
            {phase === "scan" && (
              <motion.div key="scan"
                ...
                {error && (
                  <div className="px-3 py-2.5 rounded-2xl text-[13px] font-medium"
                    style={{ background: "rgba(242,109,109,0.12)", color: "#F2685F", border: "1px solid rgba(242,109,109,0.28)" }}>
                    ⚠️ {error}
                  </div>
                )}
            ...
            {phase === "loading" && (
              <motion.div key="loading"
                ...
                <motion.div
                  ...
                  <Loader2 size={36} strokeWidth={1.5} style={{ color: "var(--exp-encre)" }} />
                </motion.div>
                <motion.p animate={{ opacity: [0.5, 1, 0.5] }} transition={{ duration: 1.6, repeat: Infinity }}
                  className="text-[13px] font-medium" style={{ color: "var(--text-2)" }}>
                  ...
                  <>
                    <div>
                      ...
                      <div className="flex gap-2">
                        ...
                        <motion.button
                          ...
                          className="flex items-center gap-1.5 px-3 py-2.5 rounded-xl text-[13px] font-semibold cursor-pointer flex-shrink-0"
                          style={{
                            ...
                            boxShadow: fallbackName.trim() && !estimating ? "var(--ombre-action)" : "none",
                      ...
                      <p className="text-[11px] mt-1.5 font-light" style={{ color: "var(--text-3)" }}>
                    ...
                    <div className="flex gap-2 mt-1">
                      <motion.button whileTap={{ scale: 0.95 }} onClick={restart}
                        ...
                        style={{ background: "rgba(var(--tint-violet-rgb),0.6)", color: "var(--exp-encre)", border: "1px solid rgba(var(--accent-rgb),0.25)" }}>
                        ↩ Rescanner
                      </motion.button>
                ...
                {estimated && (
                  <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="text-right flex-shrink-0">
                        <p className="vy-nombre text-[26px] leading-none" style={{ fontWeight: 500, color: "var(--exp-encre)" }}>{estimated.calories}</p>
                        <p className="text-[11px] mt-1" style={{ color: "var(--text-3)" }}>kcal</p>
                      </div>
                    </div>
            ...
            {phase === "result" && product && computedMacros && (
              <motion.div key="result"
                ...
                <div className="flex items-center gap-3 p-3 rounded-2xl"
                  ...
                  {product.image ? (
                    ...
                    <div className="w-14 h-14 rounded-xl flex items-center justify-center flex-shrink-0"
                      ...
                      <Barcode size={20} strokeWidth={1.5} style={{ color: "var(--exp-encre)" }} />
                    </div>
                  )}
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-[16px] leading-tight" style={{ color: "var(--text-1)" }}>
                      {product.name}
                    </p>
                    {product.brand && (
                      <p className="text-[11px] mt-0.5 font-light" style={{ color: "var(--text-2)" }}>{product.brand}</p>
                    )}
                    {product.quantity && (
                      <p className="text-[11px] mt-0.5" style={{ color: "var(--text-3)" }}>{product.quantity}</p>
                    )}
                  </div>
                  {/* Nutri-Score (si connu) */}
                  {product.nutriscore && (
                    <div className="flex flex-col items-center gap-1 flex-shrink-0">
                      <span className="text-[11px] font-bold tracking-widest" style={{ color: "var(--text-3)" }}>NUTRI</span>
                      <span className="w-8 h-8 rounded-lg flex items-center justify-center text-[16px] font-black"
                ...
                {/* Quantity stepper */}
                <div>
                  <label className="vy-label mb-2 block"
                  ...
                  <div className="flex items-center gap-2">
                    <motion.button whileTap={{ scale: 0.86 }}
                      ...
                      <Minus size={14} strokeWidth={2} style={{ color: "var(--exp-encre)" }} />
                    </motion.button>
                    <div className="flex-1 flex items-center gap-1">
                      <input
                        type="number" value={grams} min="1" max="2000"
                        onChange={e => setGrams(e.target.value)}
                        className="flex-1 text-center py-2 rounded-xl text-[16px] font-semibold outline-none"
                        style={{ background: "rgba(var(--tint-violet-rgb),0.5)", border: "1px solid rgba(var(--violet-mid-rgb),0.5)", color: "var(--text-1)" }}
                      />
                      <span className="text-[13px] font-light" style={{ color: "var(--text-3)" }}>g</span>
                    </div>
                    <motion.button whileTap={{ scale: 0.86 }}
                      onClick={() => adjustGrams(10)}
                      className="w-9 h-9 rounded-xl flex items-center justify-center cursor-pointer flex-shrink-0"
                      style={{ background: "rgba(var(--accent-rgb),0.1)", border: "1px solid rgba(var(--accent-rgb),0.2)" }}>
                      <Plus size={14} strokeWidth={2} style={{ color: "var(--exp-encre)" }} />
                    </motion.button>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between px-1">
                  <span className="text-[11px] font-medium" style={{ color: "var(--text-2)" }}>Pour {grams} g</span>
                  <span className="flex items-baseline gap-1">
                    <span className="vy-nombre text-[26px]" style={{ fontWeight: 500, color: "var(--exp-encre)" }}>{computedMacros.calories}</span>
                    <span className="text-[11px]" style={{ color: "var(--text-3)" }}>kcal</span>
                  </span>
                </div>
  ...
  return (
    <motion.div
      ...
      <motion.div
        ...
        <div className="flex items-center justify-between p-5 pb-4">
          <div>
            <p className="vy-label" style={{ color: "var(--text-3)" }}>Carte du resto</p>
            <h2 className="text-[20px] font-semibold" style={{ color: "var(--text-1)" }}>
                ...
                <div className="relative w-full rounded-2xl overflow-hidden"
                  ...
                  {camReady ? (
                    ...
                    <button type="button" onClick={() => camRef.current?.click()}
                      ...
                      <div className="text-center px-6">
                        <p className="text-[16px] font-semibold" style={{ color: "#fff" }}>Photographier la carte</p>
                        <p className="text-[11px] mt-0.5" style={{ color: "rgba(255,255,255,0.72)" }}>Autorise la caméra, ou touche pour l&apos;appareil photo</p>
                      </div>
                    </button>
                  )}
              ...
              <motion.div key="result" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold"
                    ...
                    <Target size={12} strokeWidth={2} style={{ color: "var(--exp-encre)" }} /> {objectiveChip}
                  </span>
                ...
                <div className="relative rounded-2xl p-4 overflow-hidden"
                  ...
                  <p className="font-semibold text-[20px] leading-tight mt-2.5" style={{ color: "var(--text-1)" }}>{best.name}</p>
                  {best.reason && <p className="text-[13px] mt-1 leading-snug" style={{ color: "var(--text-2)" }}>{best.reason}</p>}
                  <motion.button whileTap={{ scale: 0.97 }} onClick={() => onPickDish(best.name)}
                    className="w-full mt-3.5 py-3 rounded-2xl text-[16px] font-bold cursor-pointer flex items-center justify-center gap-2"
                    style={{ background: "linear-gradient(135deg,#8B5CF6,#C13BC1)", color: "#fff", boxShadow: "var(--ombre-action)" }}>
                        ...
                        return (
                          <motion.div key={`${d.name}-${i}`} layout
                            ...
                            <motion.button whileTap={{ scale: 0.98 }} onClick={() => setFocusedIdx(open ? null : i)}
                              ...
                              <div className="flex-1 min-w-0">
                                <p className={`font-semibold leading-tight ${open ? "text-[20px]" : "text-[16px]"}`} style={{ color: "var(--text-1)" }}>{d.name}</p>
                                ...
                                <motion.div key="cta"
                                  ...
                                  <div className="px-3 pb-3">
                                    <motion.button whileTap={{ scale: 0.97 }} onClick={() => onPickDish(d.name)}
                                      className="w-full py-2.5 rounded-xl text-[16px] font-bold cursor-pointer flex items-center justify-center gap-2"
                                      style={{ background: "linear-gradient(135deg,#8B5CF6,#C13BC1)", color: "#fff", boxShadow: "var(--ombre-action)" }}>
    ...
    <motion.div
      ...
      <motion.div
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-[20px] font-light" style={{ color: "var(--text-1)" }}>Décrire un repas</h2>
          </div>
          <motion.button whileTap={{ scale: 0.9 }} onClick={onClose}
            className="w-8 h-8 rounded-xl flex items-center justify-center cursor-pointer"
            style={{ background: "rgba(var(--tint-violet-rgb),0.8)" }}>
        ...
        <div className="flex flex-col gap-3 mb-4">
          ...
          {/* Erreur estimation */}
          {estimateError && (
            <div className="px-3 py-2 rounded-xl text-[13px]" style={{ background: "rgba(252,129,129,0.1)", color: "#E53E3E" }}>
              ⚠️ {estimateError}
            </div>
          )}
          ...
          <AnimatePresence>
            {estimated && kcal && (
              <motion.div
                ...
                <span className="text-[13px] font-medium" style={{ color: "var(--exp-encre)" }}>
            ...
            {[
              ...
              <div key={label}>
                ...
                <motion.input
                  ...
                  className="w-full px-3 py-2.5 rounded-xl text-[16px] outline-none"
                  style={{
    ...
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}
      ...
      <div className="flex items-center justify-between mb-4">
        <motion.button
          ...
          style={{ borderRadius: "var(--r-controle)", background: canPrev ? "rgba(var(--tint-violet-rgb),0.85)" : "transparent", opacity: canPrev ? 1 : 0.3,
            border: "1px solid rgba(var(--violet-mid-rgb),0.3)", boxShadow: canPrev ? "inset 0 1px 0 rgba(var(--surface-rgb),0.9)" : "none" }}
        >
        ...
        <div className="flex items-center gap-1">
          {(calMonth.getMonth() !== today.getMonth() || calMonth.getFullYear() !== today.getFullYear()) && (
            <motion.button
              ...
              className="text-[11px] font-semibold px-2.5 py-1.5 rounded-xl cursor-pointer"
          ...
          <motion.button
            ...
            disabled={!canNext}
            className="w-10 h-10 flex items-center justify-center cursor-pointer"
            style={{ borderRadius: "var(--r-controle)", background: canNext ? "rgba(var(--tint-violet-rgb),0.85)" : "transparent", opacity: canNext ? 1 : 0.3,
              border: "1px solid rgba(var(--violet-mid-rgb),0.3)", boxShadow: canNext ? "inset 0 1px 0 rgba(var(--surface-rgb),0.9)" : "none" }}
          >
          ...
          <div className="grid grid-cols-7 mb-1.5">
            {WEEK_SHORT.map((d) => (
              <div key={d} className="flex justify-center py-1">
                <span className="text-[11px] font-semibold tracking-wide" style={{ color: "var(--text-3)" }}>{d}</span>
              </div>
            ))}
          </div>
          ...
          <div className="flex items-center gap-3 mt-3 flex-wrap">
            {[
              ...
              <div key={l} className="flex items-center gap-1">
                ...
                <span className="text-[11px] font-light" style={{ color: "var(--text-3)" }}>{l}</span>
              </div>
            ))}
          </div>

          {/* ── Ce que le mois raconte, ce que tout raconte ──
             Deux trios de chiffres de MEME nature qui se suivaient a 16 px,
             l'un en trois tuiles de verre, l'autre dans un lavis violet :
             deux ecritures pour une seule idee. Un cadre, deux sections, un
             filet. Le verre est parti parce qu'il n'y avait rien derriere a
             laisser deviner : c'etait une surface statique, pas un element
             flottant. Les pictogrammes decoratifs partent avec (l'⚡ disait
             meme la serie, alors que la serie se dit 🔥 partout ailleurs). */}
          <motion.div
            initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
            className="mt-4 overflow-hidden"
            style={{ borderRadius: "var(--r-bloc)", border: "1px solid rgba(var(--text-3-rgb),0.16)" }}>

            <div className="vy-filet p-4">
              <p className="vy-label mb-3" style={{ color: "var(--text-3)" }}>Ce mois-ci</p>
              <div className="grid grid-cols-3 gap-4 text-center">
                {[
                  { label: "Jours trackés", val: `${trackedThisMonth}`, unit: `/ ${daysInMonth}`, color: "var(--text-1)" },
                  { label: "Moy. calories", val: avgCal > 0 ? avgCal.toLocaleString("fr-FR") : "—", unit: avgCal > 0 ? "kcal/j" : "", color: "var(--exp-encre)" },
                  { label: "Série en cours", val: streak > 0 ? `${streak}` : "—", unit: streak > 0 ? "jours" : "", color: "var(--feu-encre)" },
                ].map(({ label, val, unit, color }) => (
                  <div key={label}>
                    <p className="text-[20px] font-extralight leading-tight" style={{ color }}>
                      {val}
                      {unit && <span className="text-[11px] font-light ml-0.5" style={{ color: "var(--text-3)" }}>{unit}</span>}
                    </p>
                    <p className="vy-label mt-0.5" style={{ color: "var(--text-3)" }}>{label}</p>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="vy-filet p-4">
                ...
                <div className="grid grid-cols-3 gap-4 text-center">
                  {[
                    ...
                    { val: bestStreak,        unit: "jours", label: "meil. série",color: "var(--exp-encre)" },
                    { val: globalAvgCal > 0 ? globalAvgCal.toLocaleString("fr-FR") : "—",
                      unit: globalAvgCal > 0 ? "kcal" : "", label: "moy./jour",   color: "var(--feu-encre)" },
                  ].map(({ val, unit, label, color }) => (
                    <div key={label}>
                      <p className="text-[20px] font-extralight leading-tight" style={{ color }}>
                        {val}
                        <span className="text-[11px] font-light ml-0.5" style={{ color: "var(--text-3)" }}>{unit}</span>
                      </p>
                      <p className="vy-label mt-0.5" style={{ color: "var(--text-3)" }}>{label}</p>
                    </div>
                  ))}
...
export default function NutritionTab({ showBackButton = false, fullPage = true }: { showBackButton?: boolean; fullPage?: boolean }) {
  ...
  /* Le dessin commun des blocs du journal. Le halo violet a 0,16 qui vivait
     ici ne separait rien : sous une surface pleine et claire, une ombre
     teintee decore, elle ne pose pas. Les deux ombres neutres sont neutres,
     c'est le coeur du geste. Le rayon dit maintenant la taille de la surface :
     ces blocs font la largeur de l'ecran, donc `--r-bloc`. */
  const CARD = {
    background: "rgb(var(--surface-rgb))",
    ...
    <div className={fullPage ? "min-h-screen px-4 pt-[calc(env(safe-area-inset-top)+72px)] pb-36 md:pl-24 md:pr-8 md:pt-10 md:pb-10" : "w-full px-0 pt-4 pb-12"}>
        ...
        {weekDays.map((day, i) => {
          ...
          return (
            <motion.button key={i} whileTap={{ scale: 0.9 }}
              ...
              className="flex flex-col items-center gap-1 w-10 py-2 cursor-pointer"
              style={{
                borderRadius: "var(--r-controle)",
              ...
              <span className="text-[16px] font-semibold" style={{ color: isSel ? "#fff" : "var(--text-1)" }}>
                {day.getDate()}
              </span>
      ...
      {calView === "journal" && isLoading && (
        <div className="flex items-center gap-2 mb-4 max-w-5xl" style={{ color: "var(--text-3)" }}>
          ...
          <span className="text-[13px] font-light">Chargement…</span>
        </div>
      )}
      ...
      {calView === "journal" && (
        <button
          ...
          className="w-full max-w-5xl flex items-center justify-between px-5 py-4 mb-4 cursor-pointer"
          style={CARD}
        >
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0"
              style={{ background: "rgba(var(--accent-rgb),0.12)" }}>
              <BookOpen size={16} strokeWidth={1.8} style={{ color: "var(--exp-encre)" }} />
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[320px,1fr] gap-5 max-w-5xl">
        ...
        {/* ── LEFT — L'anneau et les macros, UN SEUL groupe ──
           Les deux blocs portaient EXACTEMENT le meme dessin (meme surface,
           meme rayon, meme halo violet) a 16 px l'un de l'autre, et ils
           repondent a une seule question : ou j'en suis aujourd'hui. Deux
           surfaces pour une seule idee, c'est la forme qui fait « tableau de
           bord genere ». Un cadre, deux sections, un filet entre elles.
           Le filet vient de `.vy-filet + .vy-filet`, donc c'est le SECOND
           enfant qui le porte : pas de trait orphelin si l'un disparait. */}
        <motion.div
          initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.12 }}
          className="overflow-hidden self-start w-full" style={CARD}>

          {/* Calorie ring */}
          <div className="vy-filet p-6">
            <div className="flex flex-col items-center gap-5">
              ...
              <div className="w-full grid grid-cols-3 gap-2 text-center">
                {[
                  ...
                  <div key={label}>
                    <p className="vy-label mb-0.5"
                      style={{ color: "var(--text-3)" }}>{label}</p>
                    <p className="text-[20px] font-light leading-tight" style={{ color }}>
                      {val.toLocaleString("fr-FR")}
                    </p>
                    <p className="text-[11px]" style={{ color: "var(--text-3)" }}>kcal</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="vy-filet p-5">
            <div className="flex items-start justify-between mb-4">
              ...
              <button className="text-[13px] font-semibold cursor-pointer flex-shrink-0" style={{ color: "var(--exp-encre)" }}>
                Ajuster
              </button>
            </div>
        ...
        <motion.div
          ...
          <div className="flex items-center justify-between mb-5">
            <div>
              ...
              <p className="text-[13px] font-light mt-0.5" style={{ color: "var(--text-2)" }}>
            ...
            <div className="flex gap-2">
              <motion.button
                ...
                className="flex items-center gap-1 px-3 py-2 rounded-xl text-[13px] font-semibold cursor-pointer"
                style={{
                  ...
                  return (
                    <motion.button key={i} whileTap={{ scale: 0.93 }} whileHover={{ scale: 1.03 }}
                      ...
                      <div className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0"
                        ...
                        <Plus size={11} strokeWidth={2.5} style={{ color: "var(--exp-encre)" }} />
                      </div>
                      <div className="text-left">
                        <p className="text-[11px] font-medium leading-tight flex items-center gap-1" style={{ color: "var(--text-1)", whiteSpace: "nowrap" }}>
          ...
          {meals.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-14 gap-4">
              ...
              <div className="text-center">
                ...
                <p className="text-[13px] mt-1 font-light max-w-xs" style={{ color: "var(--text-3)" }}>
                  Prends une photo, l&apos;IA identifie les aliments et remplit tout automatiquement
                </p>
              </div>
              <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.95 }}
                onClick={() => setShowPhoto(true)}
                className="flex items-center gap-2 px-5 py-3 rounded-2xl text-[16px] font-semibold cursor-pointer"
                style={{
                  background: "linear-gradient(135deg,var(--violet-mid),var(--cream-mid))",
                  color: "var(--text-1)",
                  boxShadow: "inset 0 1px 0 rgba(var(--surface-rgb),0.9)",
                    ...
                    <div className="flex flex-col">
                      {group.meals.map(meal => (
                        <motion.div key={meal.id}
                          ...
                          className="vy-filet flex items-center gap-3 py-3 group overflow-hidden">
                          ...
                          {/* Info */}
                          <div className="flex-1 min-w-0">
                            <p className="text-[16px] font-medium leading-tight truncate" style={{ color: "var(--text-1)" }}>
                              {meal.name}
                            </p>
                            <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                              <span className="text-[11px]" style={{ color: "var(--text-3)" }}>{meal.time}</span>
                              {meal.proteins > 0 && (
                                <>
                                  ...
                                  <span className="text-[11px] px-1.5 py-0.5 rounded-full hidden sm:inline-block"
                          ...
                          <div className="flex items-center gap-2 flex-shrink-0">
                            <div className="text-right">
                              <p className="vy-nombre text-[16px]" style={{ fontWeight: 700, color: "var(--text-1)" }}>{meal.calories}</p>
                              <p className="text-[11px]" style={{ color: "var(--text-3)" }}>kcal</p>
                            </div>
