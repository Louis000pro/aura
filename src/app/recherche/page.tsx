...
import { Search, User, Dumbbell, X } from "lucide-react";

type ResultType = "compte" | "contenu";

const fakeResults: { type: ResultType; name: string; sub: string }[] = [
  { type: "compte", name: "Sophie M.", sub: "Fitness · 3 abonnés communs" },
  { type: "compte", name: "Lucas Fit", sub: "Musculation · 1 abonné commun" },
  { type: "compte", name: "Marie Coach", sub: "Yoga · Coach certifiée" },
  { type: "contenu", name: "Programme full body 4j/sem", sub: "Partagé par Lucas Fit" },
  { type: "contenu", name: "Recette : bowl protéiné", sub: "Partagé par Sophie M." },
  { type: "contenu", name: "Routine mobilité matinale", sub: "Partagé par Marie Coach" },
];
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((r, i) => (
              ...
              >
                ...
                {/* Type badge */}
                <span
                  className="text-[9px] font-semibold tracking-wider uppercase px-2 py-1 rounded-full flex-shrink-0"
                  style={
                    r.type === "compte"
