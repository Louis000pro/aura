...
import { Eye, EyeOff, ArrowRight, Sparkles, User, Mail, Lock, CheckCircle2, AtSign, UserCheck, X } from "lucide-react";
import { useAuth } from "@/context/AuthContext";

type ParticleData = { id: number; x: number; y: number; size: number; delay: number; duration: number; color: string };
...
function InputField({
  ...
  return (
    <motion.div
      ...
      style={{ background: "linear-gradient(135deg,rgba(255,255,255,0.8) 0%,rgba(255,255,255,0.6) 100%)", border: "1px solid rgba(255,255,255,0.8)", backdropFilter: "blur(20px)" }}
    ...
    >
      <motion.span animate={{ color: focused ? "#A78BFA" : "#A0AEC0" }} transition={{ duration: 0.2 }}>{icon}</motion.span>
      <input
...
/* ── OAuth pseudo modal ── */
function OAuthPseudoModal({
  provider, onConfirm, onClose,
}: { provider: "google" | "apple"; onConfirm: (pseudo: string) => void; onClose: () => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      onClick={onClose}
    >
      <motion.div initial={{ scale: 0.88, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.92, opacity: 0, y: 10 }}
        transition={{ type: "spring", damping: 22, stiffness: 280 }}
        className="w-full max-w-sm rounded-3xl p-7"
        style={{ background: "rgba(255,255,255,0.92)", backdropFilter: "blur(40px)", border: "1px solid rgba(255,255,255,0.9)", boxShadow: "0 24px 64px rgba(167,139,250,0.2), inset 0 1px 0 rgba(255,255,255,0.95)" }}
        onClick={e => e.stopPropagation()}
      >
        <div className="flex items-center justify-between mb-6">
          <div>
            <p className="text-[10px] font-semibold tracking-widest uppercase" style={{ color: "#A0AEC0" }}>Continuer avec {label}</p>
            <h2 className="text-lg font-light mt-0.5" style={{ color: "#2D3748" }}>Choisissez votre pseudo</h2>
          </div>
        ...
        <motion.div className="relative flex items-center gap-3 px-4 py-3.5 rounded-2xl mb-5"
          style={{ background: "rgba(255,255,255,0.8)", border: "1px solid rgba(255,255,255,0.8)" }}
          animate={{ boxShadow: focused ? "0 0 0 2px rgba(167,139,250,0.5)" : "0 4px 16px rgba(167,139,250,0.06)" }}
        >
          <AtSign size={15} style={{ color: focused ? "#A78BFA" : "#A0AEC0" }} />
          <input autoFocus type="text" placeholder="ton_pseudo" value={pseudo}
            onChange={e => setPseudo(e.target.value.toLowerCase().replace(/\s+/g, "_"))}
            onFocus={() => setFocused(true)} onBlur={() => setFocused(false)}
            onKeyDown={e => { if (e.key === "Enter" && pseudo.trim()) onConfirm(pseudo.trim()); }}
            className="flex-1 bg-transparent text-sm outline-none placeholder:text-[#A0AEC0]"
            style={{ color: "#2D3748" }}
          />
        </motion.div>
        <motion.button whileHover={{ scale: 1.02, y: -1 }} whileTap={{ scale: 0.97 }}
          onClick={() => pseudo.trim() && onConfirm(pseudo.trim())}
          className="w-full py-3.5 rounded-2xl text-sm font-semibold cursor-pointer"
          style={{ background: pseudo.trim() ? "linear-gradient(135deg,#A78BFA 0%,#D4A843 100%)" : "rgba(220,220,220,0.5)", color: pseudo.trim() ? "#fff" : "#A0AEC0", boxShadow: pseudo.trim() ? "0 4px 20px rgba(167,139,250,0.4)" : "none" }}
        >
          Continuer
        </motion.button>
      </motion.div>
...
export default function AuthPage() {
  ...
  useEffect(() => {
    setParticles(Array.from({ length: 24 }, (_, i) => ({
      ...
      color: i % 3 === 0 ? "rgba(167,139,250,0.55)" : i % 3 === 1 ? "rgba(212,168,67,0.45)" : "rgba(240,235,255,0.7)",
    })));
  }, []);

  const canSubmit = mode === "login" ? (pseudo && password) : (pseudo && name && lastName && email && password);

  const finishLogin = async (userData: Parameters<typeof login>[0], isNew: boolean) => {
    setLoading(false);
    setSuccess(true);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await finishLogin(userData, mode === "signup");
  };

  const handleOAuth = async (provider: "google" | "apple", pseudoVal: string) => {
    setOauthModal(null);
    setLoading(true);
    await new Promise((r) => setTimeout(r, 1000));
    const fakeEmail = `${pseudoVal}@${provider === "google" ? "gmail.com" : "icloud.com"}`;
    const userData = { pseudo: pseudoVal, name: pseudoVal, lastName: "", email: fakeEmail };
    await finishLogin(userData, true);
  };
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center overflow-hidden"
      ...
      <motion.div className="absolute pointer-events-none rounded-full" style={{ width: 520, height: 520, border: "1px solid rgba(167,139,250,0.15)" }} animate={{ rotate: 360 }} transition={{ duration: 28, repeat: Infinity, ease: "linear" }} />
      <motion.div className="absolute pointer-events-none rounded-full" style={{ width: 440, height: 440, border: "1px solid rgba(212,168,67,0.12)" }} animate={{ rotate: -360 }} transition={{ duration: 20, repeat: Infinity, ease: "linear" }} />
      ...
      >
        <div className="relative rounded-3xl px-8 py-10 overflow-hidden"
          style={{ background: "linear-gradient(135deg,rgba(255,255,255,0.82) 0%,rgba(255,255,255,0.6) 100%)", backdropFilter: "blur(48px) saturate(200%)", border: "1px solid rgba(255,255,255,0.85)", boxShadow: "0 1px 0 0 rgba(255,255,255,0.95) inset,0 32px 80px -16px rgba(167,139,250,0.22),0 8px 32px -8px rgba(245,230,163,0.18)" }}
        >
          <div className="absolute top-0 left-0 right-0 h-px rounded-t-3xl" style={{ background: "linear-gradient(90deg,transparent,rgba(255,255,255,0.95),transparent)" }} />
                ...
                <motion.div className="flex gap-1.5 mt-1" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}>
                  {[0,1,2].map((i) => (
                    <motion.div key={i} className="w-1.5 h-1.5 rounded-full" style={{ background: "#2D3748" }}
                      animate={{ scale: [1,1.5,1], opacity: [0.4,1,0.4] }} transition={{ duration: 0.7, repeat: Infinity, delay: i * 0.15 }} />
                  ))}
                </motion.div>
              </motion.div>
            )}
          ...
          <div className="flex flex-col items-center mb-7">
            <motion.div className="relative w-16 h-16 rounded-2xl flex items-center justify-center mb-3"
            ...
            >
              <motion.div className="absolute inset-0 rounded-2xl" style={{ border: "1px solid rgba(167,139,250,0.5)" }} animate={{ scale: [1,1.35,1], opacity: [0.6,0,0.6] }} transition={{ duration: 2, repeat: Infinity }} />
          ...
          <div className="flex gap-2.5 mb-5">
            {[
              ...
              <motion.button key={provider} type="button"
                ...
                style={{ background: "rgba(255,255,255,0.75)", border: "1px solid rgba(255,255,255,0.9)", backdropFilter: "blur(20px)", color: "#2D3748", boxShadow: "0 2px 12px rgba(167,139,250,0.08), inset 0 1px 0 rgba(255,255,255,0.95)" }}
              >
                {icon}
                {label}
              </motion.button>
          ...
          <div className="relative flex rounded-2xl p-1 mb-5 gap-1" style={{ background: "rgba(255,255,255,0.5)", border: "1px solid rgba(255,255,255,0.7)" }}>
            {(["login","signup"] as const).map((m) => (
              <button key={m} onClick={() => setMode(m)} className="relative flex-1 py-2.5 rounded-xl text-sm font-medium cursor-pointer z-10" style={{ color: mode === m ? "#2D3748" : "#A0AEC0" }}>
                {mode === m && (
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3">
            <InputField icon={<AtSign size={15} />} type="text" placeholder="Pseudo (ex: atlas_92)" value={pseudo} onChange={setPseudo} required autoFocus />
            ...
            <InputField icon={<Lock size={15} />} type={showPassword ? "text" : "password"} placeholder="Mot de passe" value={password} onChange={setPassword} required
              suffix={
            ...
            <motion.button type="submit" disabled={loading || !canSubmit}
              ...
              style={{ background: "linear-gradient(135deg,#A78BFA 0%,#D4A843 100%)", color: "#fff", boxShadow: "0 4px 24px rgba(167,139,250,0.4),inset 0 1px 0 rgba(255,255,255,0.25)", opacity: !canSubmit ? 0.6 : 1 }}
            >
              ...
              <AnimatePresence mode="wait">
                {loading ? (
                  <motion.div key="loading" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="flex items-center justify-center gap-2">
                    <motion.div className="w-4 h-4 rounded-full border-2" style={{ borderColor: "rgba(255,255,255,0.3)", borderTopColor: "#fff" }} animate={{ rotate: 360 }} transition={{ duration: 0.8, repeat: Infinity, ease: "linear" }} />
      ...
      <AnimatePresence>
        {oauthModal && (
          <OAuthPseudoModal
            key="oauth-modal"
            provider={oauthModal}
            onConfirm={(p) => handleOAuth(oauthModal, p)}
