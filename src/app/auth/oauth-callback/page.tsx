...
function pseudoStorageKey(email: string) {
...
export default function OAuthCallbackPage() {
  ...
  const doLogin = useCallback(
    (resolvedPseudo: string, isNew: boolean) => {
      if (!session?.user) return;
      const nameParts = (session.user.name ?? "").split(" ");
      login(
        {
          pseudo: resolvedPseudo,
          name: nameParts[0] ?? resolvedPseudo,
  ...
  useEffect(() => {
    ...
    const saved = localStorage.getItem(pseudoStorageKey(email));
    if (saved) {
      doLogin(saved, false);
    } else {
      setPhase("pseudo");
    }
  }, [status, session, doLogin, router]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    const email = session.user.email ?? "";
    localStorage.setItem(pseudoStorageKey(email), p);
        ...
        {phase === "pseudo" && (
          ...
          >
            <div className="relative rounded-3xl px-8 py-10 overflow-hidden"
              style={{ background: "rgba(255,255,255,0.85)", backdropFilter: "blur(48px)", border: "1px solid rgba(255,255,255,0.9)", boxShadow: "0 32px 80px -16px rgba(167,139,250,0.22),inset 0 1px 0 rgba(255,255,255,0.95)" }}
            >
              ...
              {/* Avatar depuis Google */}
              {session?.user?.image && (
                <div className="flex justify-center mb-6">
                  <div className="relative">
                    <img src={session.user.image} alt="" className="w-16 h-16 rounded-2xl object-cover" style={{ boxShadow: "0 8px 24px rgba(167,139,250,0.3)" }} />
                    <motion.div className="absolute inset-0 rounded-2xl" style={{ border: "2px solid rgba(167,139,250,0.4)" }} animate={{ scale: [1,1.3,1], opacity: [0.7,0,0.7] }} transition={{ duration: 2, repeat: Infinity }} />
                  </div>
                </div>
              )}
              ...
              <div className="text-center mb-7">
                <h1 className="text-xl font-extralight" style={{ color: "#2D3748" }}>
                  Bonjour, <span style={{ color: "#A78BFA" }}>{session?.user?.name?.split(" ")[0]}</span> !
                </h1>
                <p className="text-xs font-light mt-1.5 leading-relaxed" style={{ color: "#718096" }}>
                  Dernière étape — choisissez votre pseudo.<br />
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                ...
                <motion.button type="submit" disabled={!pseudo.trim() || submitting}
                  whileHover={pseudo.trim() && !submitting ? { scale: 1.02, y: -2 } : {}} whileTap={{ scale: 0.97 }}
                  className="relative w-full py-4 rounded-2xl text-sm font-semibold cursor-pointer overflow-hidden"
                  style={{ background: pseudo.trim() ? "linear-gradient(135deg,#A78BFA 0%,#D4A843 100%)" : "rgba(220,220,220,0.5)", color: pseudo.trim() ? "#fff" : "#A0AEC0", boxShadow: pseudo.trim() ? "0 4px 24px rgba(167,139,250,0.4)" : "none", opacity: !pseudo.trim() ? 0.6 : 1 }}
                >
                  <AnimatePresence mode="wait">
                    {submitting ? (
                      <motion.div key="l" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex items-center justify-center gap-2">
                      ...
                      <motion.div key="i" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex items-center justify-center gap-2">
